...
function main(params) {
	...
	triangle = triangle
	    .union(flaps(params).translate([0, -side_offset]).rotateZ(a))
    ...
    for (a = 0; a < 360; a = a + 120) {
	triangle = triangle
	    .subtract(motorcut(params).translate([0, motor_offset]).rotateZ(a))
	    .union(motortab(params).translate([0, motor_offset]).rotateZ(a));
    }
    return triangle;
}
